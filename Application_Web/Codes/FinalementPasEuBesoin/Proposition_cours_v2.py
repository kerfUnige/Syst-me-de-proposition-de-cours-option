...
from GraphPrerequis.graph_prerequis import *
...
def prerequis(cours, cours_suivis, obligatoire, ajouts):
    ...
    if len(cours_possibles) == 0:
        ...
        for cours_prerequis in cours_possibles:
            if cours_prerequis[0] in cours_suivis:
                ajouts = prerequis(cours_prerequis[0], cours_suivis, obligatoire, ajouts)
            elif cours_prerequis[0] not in obligatoire:
                prerequis_dict = createGraph(cours_prerequis[0])
                for key in prerequis_dict:
                    cours_possibles = []
                    if (len(prerequis_dict.get(key)) == 0 or noeuds_suivis(prerequis_dict.get(key), cours_suivis)) and key not in obligatoire and key not in cours_suivis:
                        cours_possibles.append(key)
                    # on insère ensuite les cours dans la liste des cours à proposer en le convertissant en tuple
...
def proposition_cours(cours_suivis, periode, centre_interets):
    ...
    for cours in cours_proposees:
        ...
        for key in prerequis_dict:
            if key == cours[0] and len(prerequis_dict.get(key)) != 0 and not noeuds_suivis(prerequis_dict.get(key),
                                                                                           cours_suivis_list):
                cours_proposees = [x for x in cours_proposees if x != cours]
            cours_possibles = []
            if (len(prerequis_dict.get(key)) == 0 or noeuds_suivis(prerequis_dict.get(key), cours_suivis_list)) and key not in cours_obligatoire and key not in cours_suivis_list:
                cours_possibles.append(key)
            # on insère ensuite les cours dans la liste des cours à proposer en le convertissant en tuple
            for course in cours_possibles:
                cursor.execute(
                    f"SELECT id_cours, nom_cours, faculte, objectif, descriptif, nb_credits FROM cours WHERE id_cours='{course}'")
                cours_planning = cursor.fetchall()
                if cours_planning[0] not in cours_proposees:
    ...
    for cours in cours_suivis_list:
        cours_ajoutes = []
        cours_ajoutes = prerequis(cours, cours_suivis_list, cours_obligatoire, cours_ajoutes)
    ...
    for cours in cours_proposees:
        ...
        if len(planning_cours) == 0:
            cours_proposees = [x for x in cours_proposees if x != cours]
        elif contraintes(periode, planning_cours, obligatoire_courant):
