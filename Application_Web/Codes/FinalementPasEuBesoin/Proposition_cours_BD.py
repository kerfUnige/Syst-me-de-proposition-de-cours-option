...
def interets(centre_interets, cours_list, option_obligatoire_accompli):
    cours_proposees = []
    interets_par_nom = []
...
def proposition_cours_BD(no_immatriculation):
    ...
    cursor.execute(f"SELECT * FROM etudiant WHERE no_immatriculation = '{no_immatriculation}'")
    etudiant = cursor.fetchone()
    periode = etudiant[3]
    centre_interets = etudiant[4]
    cursor.execute(f"SELECT * FROM coursetudiant WHERE no_immatriculation = '{no_immatriculation}'")
    cours_suivis = cursor.fetchall()
    cours_suivis_list = []
    centre_interets_list = []
    if len(cours_suivis) != 0:
        for cours in cours_suivis:
            cours_suivis_list.append(cours[0])
    if centre_interets != "":
    ...
    # recherche des cours qui rentrent dans le centre d'intérêt de l'étudiant
    # on récupère une table de cours ne contenant ni les cours obligatoires, ni les cours que l'étudiant a déjà suivi
    if len(cours_suivis) == 0:
        cursor.execute(
            "SELECT id_cours, nom_cours, faculte, objectif, descriptif, option_obligatoire, nb_credits FROM cours WHERE obligatoire = 0 ")
    else:
        cursor.execute(
            f"SELECT DISTINCT cours.id_cours, nom_cours, faculte, objectif, descriptif, option_obligatoire, nb_credits FROM cours, coursetudiant WHERE obligatoire = 0 AND cours.id_cours NOT IN (SELECT id_cours FROM coursetudiant WHERE no_immatriculation = '{no_immatriculation}')")
    cours_list = cursor.fetchall()
    # on prend en compte si l'étudiant a validé un cours à option obligatoire
    cours_proposees += interets(centre_interets_list, cours_list, len(cours_proposees) == 0)
